 ...
 * Snapshot history.
 *
 * Selecting a snapshot lists the files it touches; clicking a file opens the
 * snapshot diff as a read-only tab in the main editor area. Restoring rewrites
 * files on disk, so it is always a two-click action: the first click arms the
 * button, the second confirms. There is no single-click restore anywhere.
 */
export function HistoryPanel() {
  ...
  const openDiffWorkspace = useUiStore((s) => s.openDiffWorkspace);
      ...
      <div className="flex h-full flex-col">
        <div className="flex items-center gap-2 border-b border-zinc-200 px-2 py-1 dark:border-zinc-800">
          <button
            ...
            className="rounded px-1 text-[13px] text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800"
          >
            ← History
          </button>
        ...
        <p className="px-2 pt-2 text-[10px] font-semibold tracking-wide text-zinc-400 uppercase">
          Changes — click to view the diff
        </p>

        {diffLoading && <p className="px-2 py-2 text-[13px] text-zinc-400">Comparing…</p>}

        {!diffLoading && diff.length === 0 && (
          <p className="px-2 py-2 text-[13px] text-zinc-400">
            ...
            <button
              ...
              onClick={() =>
                ...
                openDiffWorkspace({
                  ...
                  label: formatWhen(selected.createdAt),
                  initialPath: d.path,
                })
              }
          ...
          {armedId === selectedId ? (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="ml-auto rounded bg-red-600 px-2 py-1 text-[13px] font-medium text-white disabled:opacity-40"
              >
                {restoringId === selectedId ? 'Restoring…' : 'Yes, restore'}
              </button>
              <button
                onClick={() => setArmedId(null)}
                className="rounded border border-zinc-300 px-2 py-1 text-[13px] dark:border-zinc-700"
              >
                Cancel
              </button>
            </div>
            ...
            <button
              ...
              className="w-full rounded border border-zinc-300 px-2 py-1 text-[13px] hover:bg-zinc-100 disabled:opacity-40 dark:border-zinc-700 dark:hover:bg-zinc-800"
            >
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-1 border-b border-zinc-200 p-2 dark:border-zinc-800">
        <input
          ...
          className="min-w-0 flex-1 rounded border border-zinc-300 bg-white px-1.5 py-0.5 text-xs outline-none focus:border-blue-500 dark:border-zinc-700 dark:bg-zinc-900"
        />
        <button
          onClick={async () => {
          ...
          disabled={creating}
          className="shrink-0 rounded bg-blue-600 px-2 py-1 text-[13px] font-medium text-white disabled:opacity-40"
        >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {loading && <p className="px-2 py-2 text-[13px] text-zinc-400">Loading history…</p>}
        ...
        {snapshots.map((s) => (
          ...
          >
            ...
            <button
              ...
              className="shrink-0 rounded px-1 text-[13px] text-zinc-400 opacity-0 hover:bg-zinc-100 hover:text-red-600 group-hover:opacity-100 dark:hover:bg-zinc-800"
            >
              ×
            </button>
          </div>
